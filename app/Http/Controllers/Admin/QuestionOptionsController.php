...
class QuestionOptionsController extends Controller
{
    ...
    public function store(StoreQuestionOptionRequest $request)
    {
        ...
        if ($request->ajax()){
            return json_encode($questionOption , 200);
        }
    ...
    public function update(UpdateQuestionOptionRequest $request , QuestionOption $questionOption)
    {
            if ($request->ajax())
            {
                ...
                {
                    QuestionOption::where('question_id',$request->question_id )->update( [ 'is_correct' => 0] );

                    $questionOption = QuestionOption::findOrFail($request->option_id);
...
//        if($questionOption->is_correct === 0){
//            $questionOption->is_correct = 1;
//            $questionOption->save();
//            return json_encode($questionOption , 200);
//        }else {
