...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ (last(request()->segments()) == 'year') ? trans('cruds.category.fields.grade') : trans('cruds.material.title')}}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.categories.store") }}" enctype="multipart/form-data">
            @csrf
            <input class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="hidden" name="cat_type" id="cat_type" value="{{ $cat_type }}" required>
