...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" id="formUsers" action="{{ route("admin.users.store") }}" enctype="multipart/form-data"  autocomplete="off">
            @csrf
            ...
            <div class="form-group">
                <label class="required" for="email"  autocomplete="off" >{{ trans('cruds.user.fields.email') }}</label>
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email"  autocomplete="off" value="{{ old('email') }}" required >
                @if($errors->has('email'))
            ...
            <div class="form-group"  autocomplete="off">
                ...
                <input class="form-control {{ $errors->has('password') ? 'is-invalid' : '' }}" type="password" name="password"  autocomplete="off" id="password" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('gifts') ? 'is-invalid' : '' }}" name="gifts[]" id="gifts" multiple>
                    @foreach($gifts as $id => $gift)
                        <option value="{{ $id }}" {{ in_array($id, old('gifts', [])) ? 'selected' : '' }}>{{ $gift }}</option>
                    @endforeach
                </select>
                @if($errors->has('gifts'))
            ...
            <!--div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('quizzes') ? 'is-invalid' : '' }}" name="quizzes[]" id="quizzes" multiple>
                    @foreach($quizzes as $id => $quiz)
                        <option value="{{ $id }}" {{ in_array($id, old('quizzes', [])) ? 'selected' : '' }}>{{ $quiz }}</option>
                    @endforeach
                </select>
